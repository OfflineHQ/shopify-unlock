...
fn run(input: input::ResponseData) -> Result<output::FunctionResult> {
    ...
    for product_variant in cart_lines
        .iter()
        ...
        })
    {
        ...
        for gate_subject in &product_variant.product.gates {
            ...
            let gate_context_item = gate_context
                .iter()
                .find(|ctx_item| ctx_item.id.as_ref() == Some(&gate_configuration.id.to_string()));

            let gate_unlocked =
                gate_context_item.map_or(false, |ctx| is_signature_valid(ctx, gate_configuration));
...
fn parse_reaction(
    reaction: &Option<
        input::InputCartLinesMerchandiseOnProductVariantProductGatesConfigurationReaction,
    >,
) -> String {
    reaction
        .as_ref()
...
fn parse_order_limit(
    order_limit: &Option<
        input::InputCartLinesMerchandiseOnProductVariantProductGatesConfigurationOrderLimit,
    >,
) -> StringNumberOrNumber {
    order_limit.as_ref().map_or_else(
        || StringNumberOrNumber::Number(0),
        |m| {
            m.value
                .parse::<i64>()
